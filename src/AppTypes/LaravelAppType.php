...
class LaravelAppType extends AbstractAppType
{
    ...
    public function collectConfiguration(InputInterface $input, OutputInterface $output): array
    {
        ...
        if ($config[AppTypeInterface::CONFIG_INSTALL_OCTANE] === true) {
            ...
            if ($config[AppTypeInterface::CONFIG_OCTANE_SERVER] === 'swoole') {
    ...
    public function getPostInstallCommands(array $config): array
    {
        ...
        // Update modules config to include monorepo packages path
        // This allows Laravel Modules to scan the monorepo packages directory
        $commands[] = <<<'PHP'
php -r "
...
PHP;
        ...
        // Swoole check is done during configuration phase, so we just install here
