...
final class CleanupCommand extends BaseCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $this->note(
            ...
            "- All node_modules directories (npm/pnpm dependencies)\n" .
            "- All lock files (composer.lock, pnpm-lock.yaml)\n" .
            "- All caches (.turbo, .phpstan.cache, .phpunit.cache, var/cache)\n" .
            "- All build artifacts (dist, build, generated, pub/static)\n" .
            "- All log files (*.log, var/log)\n" .
            "- All temporary files (var/tmp, storage/logs)\n" .
            '- All compiled assets (bootstrap/cache, var/view_preprocessed)',
        );
        ...
        $commands = [
            ...
            // Recursively find and remove dependency directories
            ...
            "find {$root} -type d -name '.pest' -prune -exec rm -rf {} +",

            // Recursively find and remove build/dist directories
            "find {$root} -type d -name 'dist' -prune -exec rm -rf {} +",
            "find {$root} -type d -name 'build' -prune -exec rm -rf {} +",
            "find {$root} -type d -name 'generated' -prune -exec rm -rf {} +",

            // Laravel-specific cleanup
            "find {$root} -type d -path '*/bootstrap/cache' -prune -exec rm -rf {} +",
            "find {$root} -type d -path '*/storage/logs' -prune -exec rm -rf {} +",
            "find {$root} -type d -path '*/storage/framework/cache' -prune -exec rm -rf {} +",
            "find {$root} -type d -path '*/storage/framework/sessions' -prune -exec rm -rf {} +",
            "find {$root} -type d -path '*/storage/framework/views' -prune -exec rm -rf {} +",

            // Symfony-specific cleanup
            "find {$root} -type d -path '*/var/cache' -prune -exec rm -rf {} +",
            "find {$root} -type d -path '*/var/log' -prune -exec rm -rf {} +",
            "find {$root} -type d -path '*/var/tmp' -prune -exec rm -rf {} +",

            // Magento-specific cleanup
            "find {$root} -type d -path '*/pub/static/frontend' -prune -exec rm -rf {} +",
            "find {$root} -type d -path '*/pub/static/adminhtml' -prune -exec rm -rf {} +",
            "find {$root} -type d -path '*/var/view_preprocessed' -prune -exec rm -rf {} +",
            "find {$root} -type d -path '*/var/page_cache' -prune -exec rm -rf {} +",
            "find {$root} -type d -path '*/var/generation' -prune -exec rm -rf {} +",

            // Recursively find and remove lock files
            "find {$root} -name 'composer.lock' -delete",
            "find {$root} -name 'package-lock.json' -delete",
            "find {$root} -name 'yarn.lock' -delete",

            // Recursively find and remove log files
